...
public class AnalysisResource extends AlpineResource {
    ...
    public Response updateAnalysis(AnalysisRequest request) {
        ...
        failOnValidationError(
                ...
                validator.validateProperty(request, "recommendation")
        );
        try (QueryManager qm = new QueryManager()) {
            ...
            if (analysis != null) {
                if (request.getAnalysisState() != null && analysis.getAnalysisState() != request.getAnalysisState()) {
                    ...
                    analysis = qm.makeAnalysis(component, vulnerability, request.getAnalysisState(), request.isSuppressed(), request.getRecommendation());
                    ...
                    qm.makeAnalysisComment(analysis, message, commenter);
                    analysis = qm.makeAnalysis(component, vulnerability, analysis.getAnalysisState(), request.isSuppressed(), request.getRecommendation());
                } else if (request.getRecommendation() != null && !request.getRecommendation().equals(analysis.getRecommendation())) {
                    analysis = qm.makeAnalysis(component, vulnerability, analysis.getAnalysisState(), analysis.isSuppressed(), request.getRecommendation());
                }
            } else {
                analysis = qm.makeAnalysis(component, vulnerability, request.getAnalysisState(), request.isSuppressed(), request.getRecommendation());
